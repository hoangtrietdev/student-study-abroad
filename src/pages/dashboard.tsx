    ...
    >
      ...
      <div className="text-center relative z-10">
        ...
        <div className="text-xs sm:text-sm opacity-75">
          {(() => {
            const requiredSteps = section.steps.filter(
              (step) => !step.optional
            );
            // Calculate completed required steps based on percentage
            const completedRequired = Math.round(
              (percentage / 100) * requiredSteps.length
            );
            return isCompleted
...
export default function Dashboard() {
  ...
  const roadmapSections = useMemo(
    () => translateRoadmapSections(tRoadmap),
    [tRoadmap]
  );
  ...
  const {
    ...
    getSection,
    hasLoaded,
  } = useRoadmap(user, isGuestMode, roadmapSections);
  const [selectedSection, setSelectedSection] = useState<RoadmapSection | null>(
    null
  );
        ...
        <div className="flex-1 relative">
          <ReactFlow
            ...
            defaultViewport={{
              x: 0,
              y: 0,
              zoom: window.innerWidth < 768 ? 0.25 : 0.5,
            }}
          >
            <Background
              color="#374151"
